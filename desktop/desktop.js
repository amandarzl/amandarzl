...
function updateDockForWindow(win) {
  ...
  const isDocked = win.classList.contains("is-open") || win.classList.contains("is-minimized");
  dockItem.style.display = isDocked ? "flex" : "none";

  if (divider) {
    const anyDockedWindow = document.querySelectorAll('.window.is-open, .window.is-minimized').length > 0;
...
function openWindow(id) {
  ...
  const wasClosed = !win.classList.contains("is-open") &&
...
>>>>>>> 85ffab3bead5f050ecdac1ff3af8a77c64971bf2
